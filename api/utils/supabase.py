...
async def save_resume(thread_id: str, file_name: str, resume_file: File):
    ...
    file_data = {
        ...
        "create_time": str(getattr(resume_file, "create_time", None))
        if getattr(resume_file, "create_time", None)
        else None,
        "expiration_time": str(getattr(resume_file, "expiration_time", None))
        if getattr(resume_file, "expiration_time", None)
        else None,
        "update_time": str(getattr(resume_file, "update_time", None))
        if getattr(resume_file, "update_time", None)
        else None,
...
async def save_job_description(
    thread_id: str, file_name: str, job_description_file: File
):
    # Extract file attributes from the File object
    file_data = {
        # Supabase table attributes
        ...
        "create_time": str(getattr(job_description_file, "create_time", None))
        if getattr(job_description_file, "create_time", None)
        else None,
        "expiration_time": str(getattr(job_description_file, "expiration_time", None))
        if getattr(job_description_file, "expiration_time", None)
        else None,
        "update_time": str(getattr(job_description_file, "update_time", None))
        if getattr(job_description_file, "update_time", None)
        else None,
