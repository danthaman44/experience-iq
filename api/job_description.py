...
async def upload_job_description(file: UploadFile = File(...), uuid: str = Form(None)):
    try:
        ...
        await save_job_description(
            thread_id=thread_id, file_name=file_name, job_description_file=gemini_file
        )

        return JSONResponse(
            content={"message": "Job description uploaded successfully!"},
            status_code=200,
        )
...
async def delete_job_description(thread_id: str):
    try:
        ...
        return JSONResponse(
            content={"message": "Job description deleted successfully!"},
            status_code=200,
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error deleting job description: {e}"
        )
